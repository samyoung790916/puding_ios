...
@interface RBBabyNightCell()
@property (nonatomic,strong) NSArray *categoryArray;
@property (nonatomic,strong) NSArray *photoArray;
@property (nonatomic,strong) NSArray *desrArray;
@property (nonatomic,strong) UILabel *playTitle;
@end
@implementation RBBabyNightCell

-(instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        self.backgroundColor = [UIColor clearColor];
        self.contentView.backgroundColor = [UIColor clearColor];
        self.categoryArray = @[NSLocalizedString( @"night_story", nil),NSLocalizedString( @"little_animal_sleep_with_baby", nil)];
        self.desrArray = @[NSLocalizedString( @"story_description", nil),NSLocalizedString( @"story_description_2", nil)];
        [self setupSubView];
    }
    return self;
}
...
//                }
        ...
        switch (_order) {
            case 0:
                [RBStat logEvent:PD_NIGHT_ALARM_STORY message:nil];
                break;
            case 1:
                [RBStat logEvent:PD_NIGHT_ALARM_MUSIC message:nil];
                break;
            default:
                break;
        }
        @weakify(self)
        [self rb_play_type:RBSourceNight Catid:nil SourceId:[NSString stringWithFormat:@"%ld",(long)_resource.srcid] Error:^(NSString *error) {
            @strongify(self)
            if(error == nil){
                @strongify(self)
//                [self.listenBtn setSelected:YES];
