...
@interface PDMorningCallController ()<UICollectionViewDataSource,UICollectionViewDelegate,PDAlarmSettingControllerDelegate,RBUserHandleDelegate>
...
@implementation PDMorningCallController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = NSLocalizedString( @"morning_clock", nil);
    [RBStat logEvent:PD_MORNING_ALARM_VIEW message:nil];
    self.view.backgroundColor = UIColorHex(0xf7f7f7);
    self.photoArray = @[@"morning1",@"morning2",@"morning3"];
    [self setupBottomView];
    [self setupTopGuideView];
    [self fetchMoringCallData];
    [RBAlterView showMorningCallAlterView:self.view isClicked:NO];
...
-(void)alterViewHandle{
    [RBAlterView showMorningCallAlterView:self.view isClicked:YES];
}
...
-(void) setupBottomView{
    ...
    alarmTitle.text = NSLocalizedString( @"set_morning_clock_for_baby", nil);
    ...
    self.descLabel = descLabel;
     self.descLabel.text = NSLocalizedString( @"pudding_3_step_make_baby_sleep_after_set_time", nil);
...
-(void)setupTopGuideView{
    ...
    [guideCollectView registerClass:[PDMorningGuideCell class] forCellWithReuseIdentifier:NSStringFromClass([PDMorningGuideCell class])];
    self.guideCollectView = guideCollectView;
    
    UIPageControl*pageControl = [[UIPageControl alloc] init];
    [self.view addSubview:pageControl];
    pageControl.numberOfPages = self.morningAlarm.resources.count;
    pageControl.currentPageIndicatorTintColor = mRGBToColor(0x61d4ff);
...
-(void)updateCollectView{
    ...
    if (model&&[model.flag isEqualToString:@"morning"]&&(![model.status isEqualToString:@"stop"]&&![model.status isEqualToString:@"pause"])) {
...
-(void)fetchMoringCallData{
    @weakify(self)
    [RBNetworkHandle fetchEnglishAlarmWith:1 Block:^(id res) {
...
-(void)updateAlarmBtn{
    if (self.morningAlarm.alarm_id==0) {
        ...
        _alarmTitle.font = [UIFont systemFontOfSize:20];
         _alarmTitle.text = NSLocalizedString( @"set_morning_clock_for_baby", nil);
        ...
        self.descLabel.text = NSLocalizedString( @"pudding_help_baby_to_sleep_when_open_and_no_sleeping_paste_2", nil);
        if (self.morningAlarm.timer<24*3600) {
           ...
           self.dayLabel.text = [NSString stringWithFormat:NSLocalizedString( @"baby_wake_up_percent", nil),[dayFormatter stringFromDate:currDate]];
    ...
    if (self.morningAlarm.resources.count>3) {
...
-(NSString*)transformFromWithWeeks:(NSArray*)days{
    if (days.count==7) {
        return NSLocalizedString( @"baby_wake_up_every_day", nil);
    }
    NSMutableString *str = [[NSMutableString alloc] initWithString:NSLocalizedString( @"baby_wake_up__week", nil)];
...
- (CGSize)collectionView:(UICollectionView*)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath*)indexPath{
    // 107 bottomView高度 20 距离顶部高度 30 距离bottomView的距离 NAV_HEIGHT 导航栏高度
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    ...
    if (resourCount>3||resourCount==0) {
        return 3;
    }
    return resourCount;
}
-(UICollectionViewCell*) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    PDMorningGuideCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([PDMorningGuideCell class]) forIndexPath:indexPath];
    cell.order = indexPath.row;
    cell.guideImageView.image = [UIImage imageNamed:[self.photoArray objectAtIndexOrNil:indexPath.row]];
    if (self.morningAlarm.resources.count>0) {
        cell.resource = self.morningAlarm.resources[indexPath.item];
    }else{
        cell.resource = nil;
    }
//    if (indexPath.item==self.playIndex) {
//        [cell.listenBtn setSelected:YES];
//    }else{
//        [cell.listenBtn setSelected:NO];
//     }
    @weakify(self)
