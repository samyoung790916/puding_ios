...
-(void)setSkipType:(SkipType)skipType
{
    ...
    switch (skipType) {
        ...
        case SkipTypeText:
            ...
            self.timeLab.text = NSLocalizedString( @"jump", nil);
            
            break;
            
        case SkipTypeTimeText:
            
            self.timeLab.text = [NSString stringWithFormat:NSLocalizedString( @"_jump", nil),5];
...
-(void)stateWithskipType:(SkipType )skipType andDuration:(NSInteger)duration
{
    ...
    switch (skipType) {
        ...
        case SkipTypeTimeText:
             ...
             self.hidden = NO;
            self.timeLab.text = [NSString stringWithFormat:NSLocalizedString( @"__jump", nil),(long)duration];
