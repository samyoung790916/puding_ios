...
- (void)loadView
{
    ...
    __weak PECropViewController * blockself = self;
    
//    NavgationBarView * bar = [[NavgationBarView alloc] initWithFrame:CGRectMake(0, 0, self.view.width, 44 + LayoutStart)];
//    bar.showRightItem = YES;
//    [bar setRightItemTitle:@"完成"];
//    bar.showLeftItem = YES;
//    [bar setLeftItemClickBlock:^{
//        [blockself cancel:nil];
//    }];
//    [bar setRightItemClickBlock:^{
//        [blockself done:nil];
//    }];
//    [bar setTitle:@"账号管理"];
//    [self.view addSubview:bar];
    ...
    [button setTitle:NSLocalizedString( @"finnish_", nil) forState:UIControlStateNormal];
    button.frame = CGRectMake(self.view.width - 70, 20, 60, 60);
    ...
    [cancel setTitle:NSLocalizedString( @"g_cancel", nil) forState:UIControlStateNormal];
    cancel.frame = CGRectMake(10, 20, 60, 60);
    [cancel addBlockForControlEvents:UIControlEventTouchUpInside block:^(id  _Nonnull sender) {
        [blockself cancel:nil];
    }];
...
- (void)done:(id)sender
{
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSData * data = UIImageJPEGRepresentation(self.cropView.croppedImage,1.0f);
        float size = data.length/ 1024.0f;
        NSData *dataImage = nil;
        if(size > 150){
            dataImage = UIImageJPEGRepresentation(self.cropView.croppedImage, 150.f/(size));
        }else{
            dataImage = data;
        }
        
        dispatch_async(dispatch_get_main_queue(), ^{
            NSLog(@"upload image size ---> %f kb",dataImage.length/ 1024.f);
            if ([self.delegate respondsToSelector:@selector(cropViewController:didFinishCroppingImage:transform:cropRect:)]) {
                [self.delegate cropViewController:self didFinishCroppingImage:[UIImage imageWithData:dataImage] transform: self.cropView.rotation cropRect: self.cropView.zoomedCropRect];
            } else if ([self.delegate respondsToSelector:@selector(cropViewController:didFinishCroppingImage:)]) {
                [self.delegate cropViewController:self didFinishCroppingImage:[UIImage imageWithData:dataImage]];
            }
