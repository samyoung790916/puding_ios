...
@interface PDMainMenuView()<UICollectionViewDataSource,UICollectionViewDelegate>{
...
@property(nonatomic,strong) PDMainMenuHeader   *headerView;
@property (nonatomic, strong) NSString  * currentMcid; //当前主控的 Mcid

@end
@implementation PDMainMenuView
...
- (void )setContentTopOffset:(float ) topoffset{
    self.collectionView.contentInset = UIEdgeInsetsMake(topoffset + 88, 0, 0, 0);
}
...
- (void)loadHeaderView{
    PDMainMenuHeader * header = [[PDMainMenuHeader alloc] initWithFrame:CGRectMake(0, -98, SC_WIDTH, 88)];
    ...
    sepView.backgroundColor =  mRGBToColor(0xf7f7f7);;
    sepView.frame = CGRectMake(0, header.bottom , SC_WIDTH, 10);
    [self.collectionView addSubview:sepView];
...
-(void)reloadMainMenuData{
    ...
    NSString *cacheKey = [NSString stringWithFormat:@"%@%@%@%@",RB_URL_HOST,@"/home/index",@"index/modules",RBDataHandle.loginData.currentMcid];
    [RBNetworkHandle fetchMainModulesWithAge:currAge  controlID: mcid IsPlus:[deviceModel isPuddingPlus] block:^(id res) {
        @strongify(self);
        self.loadingNewData = NO;
...
//        PDBabyCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([PDBabyCollectionViewCell class]) forIndexPath:indexPath];
//        PDCategory *category = [module.categories objectAtIndexOrNil:indexPath.row];
//
//        [cell setDataSource:category];
//        return cell;
//    }else{
//        PDMainMenuCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSStringFromClass([PDMainMenuCell class]) forIndexPath:indexPath];
//        PDModule *module = [self.modulesArray objectAtIndexOrNil:indexPath.section];
